...
export function isCorrect(question: Question, answer: string): boolean {
    const trimmedAnsw: string = answer.trim();
    const lowerAnsw: string = trimmedAnsw.toLowerCase();
    const trimmedExpected: string = question.expected.trim();
    const lowerExpected: string = trimmedExpected.toLowerCase();
    return lowerAnsw === lowerExpected;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    const newquestion = {...oldQuestion, name: `Copy of ${oldQuestion.name}`, id: id, published: false}
    return newquestion;
}
